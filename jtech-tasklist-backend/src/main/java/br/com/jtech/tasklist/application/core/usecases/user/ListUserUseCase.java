/*
*  @(#)UserUseCase.java
*
*  Copyright (c) J-Tech Solucoes em Informatica.
*  All Rights Reserved.
*
*  This software is the confidential and proprietary information of J-Tech.
*  ("Confidential Information"). You shall not disclose such Confidential
*  Information and shall use it only in accordance with the terms of the
*  license agreement you entered into with J-Tech.
*
*/
package br.com.jtech.tasklist.application.core.usecases.user;

import java.util.List;
import java.util.Optional;

import br.com.jtech.tasklist.application.core.domains.User;
import br.com.jtech.tasklist.application.ports.input.user.ListUserInputGateway;
import br.com.jtech.tasklist.application.ports.output.user.ListUserOutputGateway;

/**
* class UserUseCase  
* 
* user flavio.augusto  
*/
public class ListUserUseCase implements ListUserInputGateway {
	
    private final ListUserOutputGateway outputGateway;
    
    public ListUserUseCase(ListUserOutputGateway outputGateway) {
		this.outputGateway = outputGateway;
	}

	@Override
    public List<User> listAll() {
        return outputGateway.listAll();
    }
	
	@Override
    public Optional<User> findByEmail(String email) {
        return outputGateway.findByEmail(email);
    }
}